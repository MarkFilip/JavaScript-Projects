//This function will get fired onve the DOM is loaded.
...
//---------------------------------------------
//  Initiate the game and roll for turn
//--------------------------------------------
function startGame() {
    var xTurn = 0;
...
function stopGame() {
    ...
    var btn = document.getElementById('btnStop');
    btnDisabled(btn); //disable the start button since the game is now stopped
    var showPlayer = document.getElementById('showPlayer');
    ...
    //do this to clear the running log of all game moves
...
// proposed avatar assignment and prevents them from being the same.
...
function avatarPlaced() {
    ...
    //check if there is alreay a winner...if there is, then don't continue the game
...
function glowBoard(pos) {
    ...
    for (var i=0; i<squares.length; i++) {
        if (i == index0) {
            ...
            var bg2 = squares[i];
            setTimeout( function() {bg2.style.backgroundColor = 'rgb(66, 244, 295)';}, 100);
...
function blink() {
    ...
    setTimeout(function() {body.style.backgroundColor = '#bo94f7';}, 400);
...
function checkWinCon1(info, squareArray) {
    ...
    //this will trigger (ONLY) if there was a mathc for index0, index1, and index2
